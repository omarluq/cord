...
func migrateSQLiteV1(ctx context.Context, transaction *sql.Tx) error {
	statements := []string{
		...
		"CREATE INDEX cord_nodes_status_available_at_idx ON cord_nodes(status, available_at)",
		"CREATE INDEX cord_nodes_function_status_available_at_idx ON cord_nodes(function_key, status, available_at)",
		"CREATE INDEX cord_nodes_run_status_idx ON cord_nodes(run_id, status)",
