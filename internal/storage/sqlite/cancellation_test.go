...
func TestStore_CancelRunCancelsEveryUnfinishedStateAndPreservesTerminalNodes(t *testing.T) {
	...
	plan.Nodes = []storage.Node{
		...
		newNode(plan.Run.ID, waitingNode, waitingNode, "waiting-signature", storage.NodePending, now, 4),
	...
	plan.Edges = []storage.Edge{
		...
		{RunID: plan.Run.ID, Parent: "ready", Child: waitingNode, ParentOrder: 2},
		{RunID: plan.Run.ID, Parent: "retry", Child: waitingNode, ParentOrder: 3},
	}
	...
	assertNodeState(t, database, plan.Run.ID, waitingNode, storage.NodeCanceled, 3)
